    ...
    if (item.type === 'video' && item.igUrl) {
      ...
      if (modalEmbed) {
        ...
        modalEmbed.innerHTML =
          '<a class="reel-open-btn" href="' + item.igUrl + '" target="_blank" rel="noopener">' +
          '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M8 5v14l11-7L8 5Z"/></svg>' +
          'شاهد الريلز على انستغرام</a>' +
          '<blockquote class="instagram-media" data-instgrm-permalink="' + item.igUrl +
          '" data-instgrm-version="14" style="margin:16px auto 0; max-width:400px; width:100%;">' +
          '<a href="' + item.igUrl + '" target="_blank" rel="noopener">' + item.igUrl + '</a>' +
          '</blockquote>';
